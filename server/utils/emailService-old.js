  ...
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        console.log(`📧 Resend API successful for ${email}`);
        return; // Success with Resend
      } catch (resendError) {
        console.error(`📧 Resend API failed for ${email}:`, resendError.message);
    ...
    // Fixed: Using universal sendEmail function instead
    console.log(`📧 SMTP email sent successfully to ${email}`);
  } catch (error) {
    console.error(`📧 All email methods failed for ${email}:`, error.message);
    throw error;
  }
};
...
const sendEmail = async (to, subject, html) => {
  try {
    ...
    const transporter = createTransporter();
    if (!transporter) {
      throw new Error('No email service available - configure RESEND_API_KEY or SMTP');
    }
