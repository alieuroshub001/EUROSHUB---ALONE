...
import { Search, Filter, Grid, List, Star, Calendar, Users, Clock, Eye, Briefcase } from 'lucide-react';
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter to only show projects where user is a member or client projects
        const clientProjects = data.data.projects.filter((project: Project) =>
          project.members.some(member => member.user.firstName === user?.firstName) ||
          project.owner.firstName === user?.firstName
        );
        setProjects(clientProjects);
        setTotalPages(Math.ceil(clientProjects.length / 12));
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStats({
          ...
          active: clientProjects.filter((p: Project) => p.status === 'active').length,
          completed: clientProjects.filter((p: Project) => p.status === 'completed').length,
          overdue: clientProjects.filter((p: Project) => p.isOverdue).length
        });
      }
    } catch (error) {
