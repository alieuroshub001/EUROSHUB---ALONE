...
import { projectService, Project, ProjectFilters, ProjectStatus, ProjectPriority, UserRole } from '@/lib/projectService';
import { activityService, Activity } from '@/lib/activityService';
import { useAuth } from '@/hooks/useAuth';

interface ProjectManagementProps {
  userRole: UserRole;
  baseUrl: string; // e.g., '/admin', '/hr', '/employee', '/client'
}
...
const KanbanCard: React.FC<KanbanCardProps> = ({
  ...
  return (
    <div
      ...
      draggable={canEdit}
      onDragStart={(e) => {
        if (canEdit) onDragStart?.(project);
      }}
      onDragEnd={() => {
        onDragEnd?.();
      }}
    >
      ...
      {/* Project Title */}
      <h3
        className={`font-semibold text-gray-900 mb-2 line-clamp-2 text-sm ${canEdit ? 'cursor-pointer' : ''}`}
        onClick={() => canEdit && onEdit(project._id)}
      >
        {project.title}
      </h3>
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow p-6">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h3
            className={`text-lg font-semibold text-gray-900 mb-2 line-clamp-2 ${canEdit ? 'cursor-pointer hover:text-blue-600 transition-colors' : ''}`}
            onClick={() => canEdit && onEdit(project._id)}
          >
            {project.title}
          </h3>
...
export default function ProjectManagement({ userRole, baseUrl }: ProjectManagementProps) {
  ...
  const handleCreateProject = () => {
    router.push(`${baseUrl}/project-management/create`);
  };

  // Drag and drop handlers (only for roles that can edit)
  const handleDragStart = (project: Project) => {
    if (projectService.canUserEditProject(userRole, project, user?.id || '')) {
  ...
  // Permission checks
  const canCreateProjects = projectService.canUserCreateProjects(userRole);
  const canViewAllProjects = projectService.canUserViewAllProjects(userRole);
  const canEditAllProjects = projectService.canUserEditAllProjects(userRole);
  const canDeleteAllProjects = projectService.canUserDeleteAllProjects(userRole);

  const getRoleDescription = () => {
    switch (userRole) {
      case 'superadmin': return 'Super Admin Dashboard - Full system oversight';
      case 'admin': return 'Admin Dashboard - Manage all projects and teams';
      case 'hr': return 'HR Dashboard - Oversee team projects and resource allocation';
      case 'employee': return 'Employee Dashboard - Manage assigned tasks and projects';
      case 'client': return 'Client Dashboard - View and track your projects';
      default: return 'Dashboard - Project management';
    }
  };
  ...
  const getRoleLabel = () => {
    switch (userRole) {
      case 'superadmin': return 'Super Admin Access';
      case 'admin': return 'Admin Access';
      case 'hr': return 'HR Access';
      case 'employee': return 'Employee Access';
      case 'client': return 'Client Access';
  ...
  return (
    <>
      {/* Header */}
      <div className="bg-white border-b border-gray-200 rounded-lg shadow-sm mb-6">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <span className={`px-3 py-1 text-sm font-medium rounded-full ${getRoleColor()}`}>
                {getRoleLabel()}
              </span>
            </div>
          </div>
      ...
      {/* Stats Cards - Only for admin/superadmin/hr */}
      ...
      <div className={viewMode === 'kanban' ? 'w-full' : 'grid grid-cols-1 lg:grid-cols-4 gap-8'}>
        ...
        <div className={viewMode === 'kanban' ? 'w-full' : 'lg:col-span-3'}>
                  ...
                  <div className="space-y-3 max-h-screen overflow-y-auto">
                    {projects.filter(p => p.status === 'planning').map((project) => (
                      <KanbanCard
                        ...
                        canEdit={projectService.canUserEditProject(userRole, project, user?.id || '')}
                        canDelete={projectService.canUserDeleteProject(userRole, project, user?.id || '')}
                        canArchive={projectService.canUserEditProject(userRole, project, user?.id || '')}
          ...
          {projects.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              {canCreateProjects && !searchTerm && !filters.status && !filters.priority && (
                <button
                  onClick={handleCreateProject}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                >
        ...
        {/* Sidebar - Hidden in Kanban view, and for clients/employees */}
        {viewMode !== 'kanban' && ['superadmin', 'admin', 'hr'].includes(userRole) && (
